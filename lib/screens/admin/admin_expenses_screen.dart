...
class AdminExpensesScreen extends StatefulWidget {
  ...
  @override
  State<AdminExpensesScreen> createState() => _State();
}

class _State extends State<AdminExpensesScreen> {
  List    _expenses    = [];
  List    _categories  = [];
  List    _vendors     = [];
  bool    _loading     = true;
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final eRes  = await api.get('/admin/expenses${_filterCatId != null ? '?category_id=$_filterCatId' : ''}');
      final cRes  = await api.get('/admin/general-categories');
      final vRes  = await api.get('/admin/vendors');
      dynamic eRaw = eRes['data']; if (eRaw is Map) eRaw = eRaw['data'];
      dynamic cRaw = cRes['data']; if (cRaw is Map) cRaw = cRaw['data'];
      dynamic vRaw = vRes['data']; if (vRaw is Map) vRaw = vRaw['data'];
      final list = List.from(eRaw ?? []);
      final tot  = list.fold<double>(0, (s, e) => s + (double.tryParse((e as Map)['amount']?.toString() ?? '0') ?? 0));
      setState(() { _expenses = list; _categories = List.from(cRaw ?? []); _vendors = List.from(vRaw ?? []); _total = tot; _loading = false; });
    } catch (e) { setState(() { _error = e.toString().replaceAll('Exception: ', ''); _loading = false; }); }
  }

  void _showForm({Map? expense}) {
    final titleCtrl = TextEditingController(text: expense?['title'] as String? ?? '');
    final amtCtrl   = TextEditingController(text: expense?['amount']?.toString() ?? '');
    final dateCtrl  = TextEditingController(text: expense?['expense_date'] as String? ?? DateTime.now().toIso8601String().substring(0, 10));
    final descCtrl  = TextEditingController(text: expense?['description'] as String? ?? '');
    final refCtrl   = TextEditingController(text: expense?['reference_no'] as String? ?? '');
    int?    catId   = expense?['expense_category_id'] as int?;
    String payMode  = expense?['payment_mode'] as String? ?? 'cash';
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: StatefulBuilder(builder: (ctx, setS) => SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          FormSheetHeader(icon: Icons.trending_down, title: isEdit ? 'Edit Expense' : 'Add Expense', accent: Colors.red, onClose: () => Navigator.pop(ctx)),
            ...
            onPressed: () async {
              ...
              try {
                final body = {'title': titleCtrl.text.trim(), 'amount': double.parse(amtCtrl.text.trim()),
                  'expense_date': dateCtrl.text, 'payment_mode': payMode,
                  'expense_category_id': catId, 'reference_no': refCtrl.text.trim(), 'description': descCtrl.text.trim()};
                if (isEdit) { await ApiService().put('/admin/expenses/${expense!['id']}', body); }
                else        { await ApiService().post('/admin/expenses', body); }
                if (ctx.mounted) Navigator.pop(ctx);
                _load();
                if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(isEdit ? 'Expense updated!' : 'Expense added!'), backgroundColor: Colors.green));
              } catch (e) {
  ...
  Future<void> _delete(Map exp) async {
    final ok = await AmsDialog.confirm(context, title: LanguageService.t('delete_expense'), message: 'Delete "${exp['title']}"?', icon: Icons.delete_outline_rounded, confirmText: LanguageService.t('delete'), danger: true);
    if (ok == true) {
      try { await ApiService().delete('/admin/expenses/${exp['id']}'); _load();
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(LanguageService.t('expense_deleted')), backgroundColor: Colors.red));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(LanguageService.t('expenses')),
        actions: [IconButton(icon: const Icon(Icons.refresh), onPressed: _load)],
      ),
      drawer: const AppDrawer(),
      onDrawerChanged: DrawerVisibility.onChanged,
      floatingActionButton: FloatingActionButton.extended(onPressed: () => _showForm(), icon: const Icon(Icons.add), label: Text(LanguageService.t('add_expense')), backgroundColor: Colors.red),
      body: _loading ? const Center(child: CircularProgressIndicator())
          ...
          : Column(children: [
              // Summary + filter
              Container(color: Colors.red.withOpacity(0.05), padding: const EdgeInsets.all(12), child: Column(children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
                  _S('Total Expenses', '${_expenses.length}', Icons.receipt_long, Colors.red),
                  _S('Total Amount', '$sym${_fmt(_total)}', Icons.currency_rupee, Colors.orange),
                ]),
              ...
              Expanded(child: _expenses.isEmpty
                  ? EmptyState(icon: Icons.account_balance_wallet_outlined, title: LanguageService.t('no_expenses_found'), color: Colors.red)
                  : RefreshIndicator(onRefresh: _load, child: ListView.separated(
                      padding: const EdgeInsets.fromLTRB(12, 12, 12, 90),
                      itemCount: _expenses.length, separatorBuilder: (_, __) => const SizedBox(height: 8),
                      itemBuilder: (_, i) {
                        final e    = _expenses[i] as Map;
                        final cat  = (e['category'] as Map?)?['name'] as String?;
                        return Card(child: Padding(padding: const EdgeInsets.all(12), child: Row(children: [
                          Container(width: 44, height: 44, decoration: BoxDecoration(color: Colors.red.withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
                              child: const Icon(Icons.trending_down, color: Colors.red, size: 22)),
                          const SizedBox(width: 12),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Text(e['title'] as String? ?? '—', style: const TextStyle(fontWeight: FontWeight.bold)),
                            Text('${e['expense_date'] != null ? BrandingService.formatDateString(e['expense_date'].toString()) : '—'}${cat != null ? '  ·  $cat' : ''}', style: const TextStyle(fontSize: 11, color: Colors.grey)),
                          ...
                          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                            Text('$sym${_fmt(e['amount'])}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Colors.red)),
                          ]),
                          PopupMenuButton<String>(icon: const Icon(Icons.more_vert, color: Colors.grey),
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                            onSelected: (v) { if (v == 'edit') _showForm(expense: e); if (v == 'delete') _delete(e); },
                            itemBuilder: (_) => [
